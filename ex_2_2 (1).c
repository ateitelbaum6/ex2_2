        ...
        switch(choice){
            ...
            case addAll: {
                ...
                while (total_filled < NUM_OF_BRANDS) {
                    ...
                    if (target_brand >= 0 && target_brand < NUM_OF_BRANDS && filled_brands[target_brand] == 0) {
                        if (scanf("%d %d %d %d", &cube[0][target_brand][0], 
                                                 &cube[0][target_brand][1], 
                                                 &cube[0][target_brand][2], 
                                                 &cube[0][target_brand][3]) == 4) {
            ...
            case stats: {
                ...
                if (scanf("%d", &day) == 1 && day >= 1 && day <= DAYS_IN_YEAR) {
                    int total_sales = 0;
                    int highest_brand_sales = -1, best_brand_idx = 0;
                    int highest_type_sales = -1, best_type_idx = 0;
                    ...
                    for (int t = 0; t < NUM_OF_TYPES; t++) {
                        ...
                        if (has_data && type_sum > highest_type_sales) {
                    ...
                    printf("The best-selling brand is %s: %d\n", brands[best_brand_idx], highest_brand_sales == -1 ? 0 : highest_brand_sales);
                    printf("The best-selling type of car is %s: %d\n", types[best_type_idx], highest_type_sales == -1 ? 0 : highest_type_sales);
                } else {
                    printf("Invalid input\n");
                    while (getchar() != '\n');
                }
            ...
            case insights: {
                ...
                for (int b = 0; b < NUM_OF_BRANDS; b++) {
                    ...
                    if (has_data && brand_total > max_brand_sales) {
                ...
                for (int t = 0; t < NUM_OF_TYPES; t++) {
                    ...
                    if (has_data && type_total > max_type_sales) {
                ...
                for (int d = 0; d < DAYS_IN_YEAR; d++) {
                    ...
                    if (has_data && day_total > max_day_sales) {
                        max_day_sales = day_total;
                        overall_best_day = d;
                    }
                }

                printf("The best-selling brand overall is %s: %d\n", brands[overall_best_brand], max_brand_sales == -1 ? 0 : max_brand_sales);
                printf("The best-selling type of car is %s: %d\n", types[overall_best_type], max_type_sales == -1 ? 0 : max_type_sales);
                printf("The most profitable day was day number %d: %d\n", overall_best_day + 1, max_day_sales == -1 ? 0 : max_day_sales);
                break;
            }
            case deltas: {
                for (int b = 0; b < NUM_OF_BRANDS; b++) {
                    ...
                    for (int d = 1; d < DAYS_IN_YEAR; d++) {
                        int today_total = 0, yesterday_total = 0;
                        int today_has_data = 0, yesterday_has_data = 0;
